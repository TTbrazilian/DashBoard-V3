...
st.set_page_config(page_title="Gestão de Recursos - Itaú de Minas", layout="wide")
...
# ── CARGA DE DADOS ────────────────────────────────────────────────────────────
# Especificidades Itaú de Minas:
# • Itaú_de_Minas.csv: mesmo padrão Bom Jesus — header=1, colunas triplicadas.
#   'Abril .1' com espaço → rename_liq explícito. Linhas NaN → filtradas.
# • 6 categorias: Secretaria, Atenção Primária, MAC, Farmácia,
#                 Atenção Psicossocial, Vigilância.
# • Capital Liq Jan–Mar = R$ 85.119,11 | Custeio = R$ 7.180.469,31
# • Total Liq Jan–Mar = R$ 7.265.588,42
# • Itaú_de_Minas_DF.csv: tem linha Tipo='Tipo' → filtrada via isin().
#   Fonte Própria = 1500 (não 1600/1601...).
# • Itaú_de_Minas_R.csv: header=0 direto, colunas já corretas.
#   Categorias: Impostos, Cota-Parte (sem FUNDEB/Vinculados).
@st.cache_data
def load_data():
    ...
    path_f  = _buscar("Itaú de Minas.csv")
    path_r  = _buscar("Itaú de Minas_R.csv")
    path_df = _buscar("Itaú de Minas_DF.csv")
...
if df_raw is not None:
    ...
    # 6 categorias: inclui Atenção Psicossocial
    NOME_CATEGORIA = {
    ...
    fig_pizza_cat.update_layout(
        ...
        height=500,
        separators=',.',
        margin=dict(t=60, b=40, l=20, r=210),
