...
if df_f_raw is not None and df_r is not None:
    ...
    if st.session_state.setor == 'FUNDEB':
        ...
        if view_comp == "Acumulado":
            # Cálculo dos valores totais para o acumulado
            ...
            dados_acum = [
                {"Tipo": "Receita Total", "Valor": r_total, "Texto": ""},
                {"Tipo": "Despesas (70%)", "Valor": d_total, "Texto": f"{perc_total:.2f}%"}
            ]
            
            fig_comp = px.bar(pd.DataFrame(dados_acum), x='Tipo', y='Valor', color='Tipo',
                              text='Texto',
            ...
            # Lógica Mensal (onde a variável dados_m_comp é preenchida)
            dados_m_comp = []
            for m in meses_disponiveis:
