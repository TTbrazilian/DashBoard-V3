    ...
    # =========================================================================
        ...
        if view_rp == "Acumulado":
            if ativo == "Acumulado Geral":
                df_plot = df_r_grupo.copy().sort_values('Janeiro', ascending=False)
                # Soma todos os meses disponíveis para ordenar
                df_plot['Total'] = df_plot[meses_disponiveis].sum(axis=1)
                df_plot = df_plot.sort_values('Total', ascending=False)
                fig_rp = px.bar(df_plot, x='Grupo', y=meses_disponiveis[0] if len(meses_disponiveis)==1
                                else df_plot['Total'],
                                color='Grupo', text_auto='.3s',
                                color_discrete_map=mapa_cores_grupos)
                # Refazer com valor total acumulado
        ...
        if val_outras_fontes > 0:
            st.info(f"ℹ️ **Outras despesas de anos anteriores (Fonte 1500 — Liquidado):** "
                    f"{formar_real(val_outras_fontes)} — não compõem o índice vigente.")
        st.markdown("---")
