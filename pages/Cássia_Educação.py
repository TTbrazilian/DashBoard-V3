...
@st.cache_data
def load_all_data():
    arquivo_f = "zEducação/Cássia.csv"
    arquivo_r = "zEducação/Cássia_R.csv"
    arquivo_df = "zEducação/Cássia_DF.csv"
...
if df_f_raw is not None and df_r is not None:
    ...
    if st.session_state.setor == 'FUNDEB':
        st.markdown("<h1 style='text-align: left;'>📖 Cássia - FUNDEB</h1>", unsafe_allow_html=True)
    ...
    # --- SETOR RECURSOS PRÓPRIOS ---
        ...
        st.markdown("<h1 style='text-align: left;'>📖 Cássia - Recursos Próprios (25%)</h1>", unsafe_allow_html=True)
        ...
        fig_d.update_traces(
            hovertemplate="<span style='color:white;'><b>%{x}</b><br>Status: %{fullData.name}<br>Valor Investimento: R$ %{customdata[2]:,.2f}<br>Dedução: R$ %{customdata[1]:,.2f}<br>Proporção: %{customdata[0]}</span><extra></extra>",
            hoverlabel=HOVER_STYLE
        )
        ...
        if view_meta == "Acumulado":
            ...
            fig_meta.add_hline(y=total_rec_base * 0.25, line_dash="dash", line_color="#f39c12", annotation_text="Meta 25%", annotation_position="top left")
        else:
            dados_meta_m = []
            for m in meses_disponiveis:
                r_m = df_r_base[m].sum()
            ...
            fig_meta.add_trace(go.Scatter(x=df_linha_meta['Mês'], y=df_linha_meta['Meta Mensal (25%)'], mode='lines+markers', name='Meta 25%', line=dict(color='#f39c12', dash='dash')))
    ...
    # --- SETOR RECURSOS VINCULADOS ---
        ...
        st.markdown("<h1 style='text-align: left;'>📖 Cássia - Recursos Vinculados</h1>", unsafe_allow_html=True)
        ...
        for prog in programas:
            ...
            if tipo_vinc == "Acumulado":
                rec = df_prog_r[meses_reais_r].sum().sum()
                ...
                fig_vinc = px.bar(df_acum, x='Tipo', y='Valor', color='Tipo', text_auto='.2s', color_discrete_map={'Receita':'#002147', 'Despesa':'#660000'})
            else:
                dados_d_v = []
                for m in meses_disponiveis:
                    c_r, c_df = colunas_limpas_r.get(m), colunas_limpas_df.get(m)
                    rec_m = df_prog_r[c_r].sum() if c_r else 0
                ...
                fig_vinc = px.bar(pd.DataFrame(dados_d_v), x='Mês', y='Valor', color='Tipo', barmode='group', color_discrete_map={'Receita':'#002147', 'Despesa':'#660000'}, category_orders={"Mês": ORDEM_MESES})

            fig_vinc.update_traces(selector=dict(type='bar'), hovertemplate="<b>%{x}</b><br>Valor: R$ %{y:,.2f}<extra></extra>", hoverlabel=HOVER_STYLE, textposition='outside')
            fig_vinc.update_layout(separators=",.", yaxis_title="Valor (R$)", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    ...
    st.error("Erro ao carregar os arquivos CSV. Verifique se os arquivos 'Cássia.csv', 'Cássia_R.csv' e 'Cássia_DF.csv' estão na pasta 'zEducação'.")