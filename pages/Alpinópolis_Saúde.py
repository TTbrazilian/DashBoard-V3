...
# ── CARGA DE DADOS ────────────────────────────────────────────────────────────
# Especificidades Alpinópolis:
# • Alpinópolis.csv: mesmo padrão de Bom Jesus — header=1, colunas triplicadas
#   por mês (.1 = Liquidado, .2 = Pago). 'Abril .1' tem espaço antes do ponto.
# • 8 categorias: MAC, Atenção Primária, ADM Geral Vinculada á Saúde, Secretaria,
#   Formação Recursos Humanos, Farmácia, Vigilância, Alimentação e Nutrução.
# • Capital (Equipamentos + Obras) Liq = R$ 1.420,00 | Custeio = R$ 7.882.407,87
# • Total Liq Jan–Mar = R$ 7.883.827,87
# • Alpinópolis_DF.csv: colunas Tipo/Nomenclatura/Fonte (sem cabeçalho repetido).
#   Coluna 'Março ' tem espaço no final → strip no rename.
# • Alpinópolis_R.csv: Categorias = Impostos, Cota-Parte (com espaço no final).
#   Coluna 'Março ' tem espaço no final → strip.
@st.cache_data
def load_data():
    ...
    path_f  = _buscar("Alpinópolis.csv")
    path_r  = _buscar("Alpinópolis_R.csv")
    path_df = _buscar("Alpinópolis_DF.csv")
    ...
    # Renomeia colunas Liquidado (.1) — 'Abril .1' com espaço tratado explicitamente
    rename_liq = {
    ...
    # ── RECEITAS ─────────────────────────────────────────────────────────────
    # 'Março ' com espaço no final → stripped no rename de colunas
    df_r = None
    if path_r:
    ...
    if path_df:
        ...
        # Filtra apenas linhas com estágio válido (remove linhas extras/vazias)
...
if df_raw is not None:
    ...
    # Alpinópolis tem 8 categorias — mapeadas para nomes de exibição
    NOME_CATEGORIA = {
        ...
        'ADM Geral Vinculada á Saúde':   'ADM Geral Vinculada à Saúde',
        'Formação Recursos Humanos':     'Formação de Recursos Humanos',
        'Alimentação e Nutrução':        'Alimentação e Nutrição',
    }
    CORES_CATEGORIA = {
        'Secretaria / Administração':        '#f39c12',
        'Ambulatorial e Especialidades (MAC)':'#1a3a6b',
        'Atenção Primária':                  '#4A90D9',
        'Farmácia':                          '#27ae60',
        'Vigilância':                        '#e74c3c',
        'ADM Geral Vinculada à Saúde':       '#8e44ad',
        'Formação de Recursos Humanos':      '#16a085',
        'Alimentação e Nutrição':            '#d35400',
    }
    ...
    fig_pizza_cat.update_layout(
        ...
        height=500,
        separators=',.',
        margin=dict(t=60, b=40, l=20, r=220),
