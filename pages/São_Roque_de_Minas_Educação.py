...
COR_DESP = {
    ...
    'VAAR – Vigente':       '#6a0dad',   # roxo — despesas VAAR (fonte 1543+2543)
}
...
# ── CARGA DE DADOS ────────────────────────────────────────────────────────────
# Individualidades São Roque vs Alpinópolis:
# • R file: header=0 direto | coluna orçado = 'Orçamento Receitas' (≠ Alpinópolis)
# • VAAR tem valores reais: R$178.167,64 (Jan–Abr) + Repasse = R$501.227,71
# • VAAR NÃO entra na base 70% | ETI base inclui VAAR
# • Despesas VAAR: fontes 1543 (R$73.345,35) + 2543 (R$37.431,32)
#   → exibidas no gráfico Receitas e Despesas FUNDEB (cor roxa, label 'VAAR – Vigente')
# • QESE fontes DF: apenas ['1550'] (sem 2550)
# • Remunerações Bancárias: 4 linhas todas com desc='Rendimentos' (sem nome de programa)
#   → tratadas como total genérico; hover mostra soma total por programa
# • Folha: sem 'Auxílio-alimentação' | nova fonte 25001 (Rec. Próprios Superávit)
# • Descontos 25%: 0,0
# • Capital Liq = R$356.422,50 | Custeio = R$4.584.975,93
# • FUNDEB 70% Liq = R$1.802.018,58 | FUNDEB 30% = R$175,61 | VAAR Liq = R$73.345,35
# • RP (15001+1500) Liq = R$1.545.495,65
@st.cache_data
def load_all_data():
    path_f  = buscar_arquivo("zEducação/São Roque de Minas.csv")
    path_r  = buscar_arquivo("zEducação/São Roque de Minas_R.csv")
    path_df = buscar_arquivo("zEducação/São Roque de Minas_DF.csv")
    ...
    # Coluna orçado = 'Orçamento Receitas' (diferente de Alpinópolis)
    # Repasse e 2025 incluídos → numéricos (evita erro nos Vinculados)
    # Atualização Quadrimestral: fora de COLS_R → limpeza 1x no bloco FUNDEB
    df_r = pd.read_csv(path_r, sep=None, engine='python', encoding='utf-8', header=0)
    df_r.columns = [str(c).strip() for c in df_r.columns]
    COLS_R = ORDEM_MESES + ['Total','Orçamento Receitas','Repasse','2025']
...
if df_f_raw is not None and df_r is not None:
    ...
    if st.session_state.setor == 'FUNDEB':
        st.markdown("<h1 style='text-align:left;'>📖 São Roque de Minas — FUNDEB</h1>",
                    unsafe_allow_html=True)
        ...
        # Coluna orçado = 'Orçamento Receitas' | Portaria separada para Principal e VAAR
        ...
        desp_70_vigente = soma(df_df_fundeb_70_30[(df_df_fundeb_70_30['Fonte']=='15407') &
                                (df_df_fundeb_70_30['Tipo']=='Liquidado')], meses_disponiveis)
        desp_30_vigente = soma(df_df_fundeb_70_30[(df_df_fundeb_70_30['Fonte']=='15403') &
                                (df_df_fundeb_70_30['Tipo']=='Liquidado')], meses_disponiveis)
        desp_vaar_vigente = soma(df_df_fundeb_vaar[df_df_fundeb_vaar['Tipo']=='Liquidado'],
                                  meses_disponiveis)
        tot_desp_vigente = desp_70_vigente + desp_30_vigente + desp_vaar_vigente
        ...
        with t2: st.metric("Total Despesas Liquidadas – Ano Vigente (70%+30%+VAAR)",
        ...
        if tipo_rd == "Mensal":
            for m in meses_disponiveis:
                ...
                tot_desp_m = (soma(df_df_fundeb_70_30[df_df_fundeb_70_30['Tipo']=='Liquidado'], [m]) +
                              soma(df_df_fundeb_vaar[df_df_fundeb_vaar['Tipo']=='Liquidado'], [m]))
                ...
                # Despesas: 70%, 30% e VAAR
                for fonte_cod, label_desp, df_src in [
                    ('15407', 'FUNDEB 70% – Vigente', df_df_fundeb_70_30),
                    ('15403', 'FUNDEB 30% – Vigente', df_df_fundeb_70_30),
                    ('1543+2543', 'VAAR – Vigente',   df_df_fundeb_vaar),
                ]:
                    if fonte_cod == '1543+2543':
                        val = soma(df_src[df_src['Tipo']=='Liquidado'], [m])
                    else:
                        val = soma(df_src[(df_src['Fonte']==fonte_cod) &
                                          (df_src['Tipo']=='Liquidado')], [m])
                    if val == 0 and label_desp not in legendas_usadas: pass
                    ...
                    fig_rd.add_trace(go.Bar(
                        ...
                        textposition='outside' if label_desp=='FUNDEB 30% – Vigente' else 'inside',
            ...
            for fonte_cod, label_desp, df_src in [
                ...
                if fonte_cod == '1543+2543':
                    val = soma(df_src[df_src['Tipo']=='Liquidado'], meses_disponiveis)
                else:
                    val = soma(df_src[(df_src['Fonte']==fonte_cod) &
                                      (df_src['Tipo']=='Liquidado')], meses_disponiveis)
        ...
        st.caption("ℹ️ Base 70% = Principal + Rendimentos + VAAT. VAAR excluído conforme legislação.")
        ...
        if tipo_70 == "Acumulado":
            ...
            fig_70.add_trace(go.Bar(
                ...
                hovertemplate=("<span style='color:white;'><b>Receita Base FUNDEB</b><br>"
                               "Principal + Rendimentos + VAAT (VAAR excluído)<br>"
            ...
            for m in meses_disponiveis:
                ...
                liq_70_m = soma(df_df_fundeb_70_30[(df_df_fundeb_70_30['Fonte']=='15407') &
                                             (df_df_fundeb_70_30['Tipo']=='Liquidado')], [m])
        ...
        # ETI base inclui VAAR
        st.subheader("🔹 3. Percentual Tempo Integral")
        meta_eti_perc = 4.0
        _base_eti = soma(
            df_r_fundeb[df_r_fundeb['Subcategoria'].isin(
                ['Principal','Rendimentos','VAAT','VAAR'])],
            meses_disponiveis
        )
    ...
    # =========================================================================
        ...
        st.markdown("<h1 style='text-align:left;'>📖 São Roque de Minas — Recursos Próprios (25%)</h1>",
                    unsafe_allow_html=True)
        ...
        prev_total_rp = df_r_base['Orçamento Receitas'].sum() if 'Orçamento Receitas' in df_r_base.columns else 0.0
    ...
    # Rendimentos: todos rotulados "Rendimentos" sem nome de programa
    # → somados como total genérico; sem matching por nome
    # =========================================================================
    elif st.session_state.setor == 'Recursos Vinculados':
        st.markdown("<h1 style='text-align:left;'>📖 São Roque de Minas — Recursos Vinculados</h1>",
                    unsafe_allow_html=True)
        ...
        total_rec_vinc_prog = soma(
            df_r[df_r['Descrição da Receita'].str.strip().str.upper().isin(programas)],
            meses_disponiveis
        )
    ...
    # =========================================================================
        ...
        st.markdown("<h1 style='text-align:left;'>📖 São Roque de Minas — Visão Macro da Educação</h1>",
                    unsafe_allow_html=True)
        st.markdown("---")
    ...
    # São Roque: sem Auxílio-alimentação | 25001 = Rec. Próprios Superávit
    # =========================================================================
    elif st.session_state.setor == 'Folha de Pagamento':
        st.markdown("<h1 style='text-align:left;'>📖 São Roque de Minas — Folha de Pagamento</h1>",
                    unsafe_allow_html=True)
        st.markdown("---")
        ...
        def origem_fonte(f):
            ...
            if f == '25001': return 'Rec. Próprios (25001)'
        ...
        total_rp     = df_folha[df_folha['Origem'].str.startswith('Rec. Próprios')][liq_cols_f].sum().sum()
        ...
        with f2: st.metric("FUNDEB 70% (Fonte 15407)", formar_real(total_fund70),
                           ...
                           delta_color="off")
        with f3: st.metric("Recursos Próprios (15001/25001/1500)", formar_real(total_rp),
