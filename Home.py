...
if df_f_raw is not None and df_r is not None:
    ...
    if st.session_state.setor == 'FUNDEB':
        ...
        if not df_fig_r.empty and df_fig_r['Valor'].sum() > 0:
            ...
            fig_r_bar.update_yaxes(title="Porcentagem (%)", tickmode='linear', tick0=0, dtick=10)
            st.plotly_chart(fig_r_bar, use_container_width=True, config=CONFIG_PT)
        else:
            st.info("Aguardando dados de Receitas para geração do gráfico.")
        ...
        if not df_fig_f.empty and df_fig_f['Valor'].sum() > 0:
            ...
            fig_f_bar.update_yaxes(title="Porcentagem (%)", tickmode='linear', tick0=0, dtick=10)
            st.plotly_chart(fig_f_bar, use_container_width=True, config=CONFIG_PT)
        else:
            st.info("Aguardando dados de Despesas para geração do gráfico.")
