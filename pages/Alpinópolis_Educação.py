...
# Configuração global de separadores: "." para milhar e "," para decimal
CONFIG_PT = {'displaylogo': False, 'showTips': False, 'locale': 'pt-BR'}
pio.layout_template_default = "plotly_white"
...
if df_f_raw is not None and df_r is not None:
    ...
    if st.session_state.setor == 'FUNDEB':
        ...
        fig_r.update_traces(hovertemplate='Mês=%{x}<br>Categoria=%{fullData.name}<br>Valor=%{y:,.2f}<extra></extra>')
        ...
        fig_f.update_traces(hovertemplate='Mês=%{x}<br>Fonte=%{fullData.name}<br>Valor=%{y:,.2f}<extra></extra>')
        ...
        if tipo_grafico == "Total Acumulado":
            ...
            fig_comp = px.bar(pd.DataFrame(dados_m_comp), x='Mês', y='Valor', color='Tipo', barmode='group', text_auto='.2s',
                              color_discrete_map={"Receitas (Base)": "#003366", "Despesas (70%)": "#660000"})
            fig_comp.update_traces(hovertemplate='Mês=%{x}<br>Tipo=%{fullData.name}<br>Valor=%{y:,.2f}<extra></extra>')
