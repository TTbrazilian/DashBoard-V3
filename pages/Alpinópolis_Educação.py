...
if df_f_raw is not None and df_r is not None:
    ...
    if st.session_state.setor == 'FUNDEB':
        ...
        # Análise percorrendo os três arquivos para Jan-Mar
        tot_rec_ano = df_r_fundeb[meses].sum().sum()
        tot_prev_2026 = df_r_fundeb['Orçado Receitas'].sum()
        rec_base_70 = df_r_fundeb[df_r_fundeb['Subcategoria'] != 'VAAR'][meses].sum().sum()
        desp_70_val = df_df_fundeb[(df_df_fundeb['Fonte_Nome'] == 'FUNDEB 70%') & (df_df_fundeb['Tipo'] == 'Liquidado')][meses].sum().sum()
        ...
        fig_r_bar = px.bar(df_plot_r, x='Mês', y='Valor', color='Categoria', 
                           text='Valor', # Valor real na barra
                           color_discrete_map={'Principal':'#636EFA', 'VAAR':'#00CC96', 'ETI':'#EF553B', 'Aplicação':'#AB63FA'})
        fig_r_bar.update_layout(barnorm='percent', separators=',.') # Porcentagem apenas na lateral
        ...
        fig_f_bar = px.bar(df_plot_f, x='Mês', y='Valor', color='Fonte', 
                           text='Valor', # Valor real na barra
                           color_discrete_map={'FUNDEB 70%':'#4169E1', 'FUNDEB 30%':'#FF4500'})
        fig_f_bar.update_layout(barnorm='percent', separators=',.') # Porcentagem apenas na lateral
        ...
        st.markdown("---")
        st.subheader("🔹 3. Análises e Equilíbrio (Jan - Mar)")
        total_desp_liq = df_df_fundeb[df_df_fundeb['Tipo'] == 'Liquidado'][meses].sum().sum()
        ...
        col_liq_fichas = [c for c in df_f.columns if any(m in c for m in meses) and 'Liquidado' in c]
    ...
    # --- PÁGINA: RECURSOS PRÓPRIOS ---
        ...
        # Código de Recursos Próprios preservado...
        ...
        fig_desp = px.bar(pd.DataFrame(dados_desp_15001), x='Mês', y='Valor', color='Tipo', barmode='group')
    ...
    # --- PÁGINA: RECURSOS VINCULADOS ---
        ...
        # Código de Recursos Vinculados preservado...
