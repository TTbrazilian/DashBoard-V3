...
st.markdown(
    ...
    <style>
        ...
        /* LÓGICA DE FOCO REAPLICADA */
        .stButton button:focus {
            border-color: #002147 !important;
            color: #002147 !important;
            box-shadow: 0 0 0 0.2rem rgba(0,33,71,.25) !important;
        }
...
def formar_real(valor):
    """Retorna o formato xx.xxx,xx conforme solicitado para o hover"""
...
if df_f_raw is not None and df_r is not None:
    ...
    if st.session_state.setor == 'FUNDEB':
        ...
        fig_r.update_traces(hovertemplate="<b>%{x}</b><br>%{fullData.name} = %{customdata}<extra></extra>", customdata=df_plot_r['F'])
        ...
        fig_f.update_traces(hovertemplate="<b>%{x}</b><br>%{fullData.name} = %{customdata}<extra></extra>", customdata=df_plot_f['F'])
        ...
        if tipo_grafico == "Total Acumulado":
            ...
            df_plot_comp = pd.DataFrame(dados_m_comp)
            fig_comp = px.bar(df_plot_comp, x='Mês', y='Valor', color='Tipo', barmode='group',
                              color_discrete_map={"Receitas (Base)": "#003366", "Despesas (70%)": "#660000"})
            fig_comp.update_traces(hovertemplate="<b>%{x}</b><br>%{fullData.name} = %{customdata}<extra></extra>", customdata=df_plot_comp['F'])
        ...
        if 'rp_ativo' in st.session_state:
            ...
            df_plot_rp = pd.DataFrame(dados_r_mensal)
            fig_r_prop = px.bar(df_plot_rp, x='Mês', y='Valor', color='Tipo', barmode='group',
                               color_discrete_map={"Receita Mensal": "#003366", "Dedução": "#6699cc"})
            fig_r_prop.update_traces(hovertemplate="<b>%{x}</b><br>%{fullData.name} = %{customdata}<extra></extra>", customdata=df_plot_rp['F'])
        ...
        df_plot_desp = pd.DataFrame(dados_d_mensal)
        fig_d_prop = px.bar(df_plot_desp, x='Mês', y='Valor', color='Fase', barmode='group',
                           color_discrete_map={"Empenhado": "#660000", "Liquidado": "#cc0000", "Pago": "#ff4d4d"})
        fig_d_prop.update_traces(hovertemplate="<b>%{x}</b><br>%{fullData.name} = %{customdata}<extra></extra>", customdata=df_plot_desp['F'])
