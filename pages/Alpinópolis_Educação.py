...
# ── CARGA DE DADOS ────────────────────────────────────────────────────────────
# Individualidades Alpinópolis vs Pratápolis:
# • Alpinópolis_R.csv: header=0 DIRETO (linha 0 já tem os nomes das colunas).
#   Coluna orçado = 'Orçado Receitas'. Sem Repasse e sem Atualização Quadrimestral.
# • VAAT / VAAR / ETI = R$ 0 no período. Base 70% = Principal + Rendimentos.
# • Rendimentos FUNDEB = R$ 46.462,20 (Jan–Abr). Incluídos na base 70%.
# • Alpinópolis_DF.csv: tem linha Tipo='Tipo' → filtrar via isin().
# • QESE fontes DF: ['1550','2550'] (Superávit QESE = 2550).
# • Fontes próprias: 15001 + 1500 (ambas compõem o 25%).
# • PTE: categoria 'Tranferência Programas Estaduais' no R file.
# • Rendimentos bancários (Remunerações Bancárias): 4 linhas (PNAE, PNATE, PTE, QESE)
#   → somados à receita de cada programa no gráfico; hover mostra Transferência + Rendimentos.
# • meses_disponiveis = ['Janeiro','Fevereiro','Março','Abril'] (4 meses).
# • Capital Liq = R$ 83.357,21 | Custeio = R$ 6.654.813,23
# • FUNDEB 70% Liq = R$ 3.794.348,68 | FUNDEB 30% = R$ 808.885,98
# • RP (15001+1500) Liq = R$ 1.818.989,22 | _desconto = 0,0
@st.cache_data
def load_all_data():
    path_f  = buscar_arquivo("zEducação/Alpinópolis.csv")
    path_r  = buscar_arquivo("zEducação/Alpinópolis_R.csv")
    path_df = buscar_arquivo("zEducação/Alpinópolis_DF.csv")
    ...
    # ── RECEITAS — header=0 DIRETO (linha 0 já é o cabeçalho) ────────────────
    ...
    # ── DESPESAS POR FONTE — filtrar Tipo='Tipo' via isin() ──────────────────
...
if df_f_raw is not None and df_r is not None:
    ...
    if st.session_state.setor == 'FUNDEB':
        st.markdown("<h1 style='text-align:left;'>📖 Alpinópolis — FUNDEB</h1>",
                    unsafe_allow_html=True)
        ...
        # Coluna orçado = 'Orçado Receitas' | Repasse = 0 | sem Atualização Quad.
        tot_prev_municipio = df_r_fundeb[df_r_fundeb['Subcategoria']=='Principal'
                                         ]['Orçado Receitas'].sum()
        tot_prev_portaria  = df_r_fundeb['Repasse'].sum() if 'Repasse' in df_r_fundeb.columns else 0.0
        ...
        # Base 70% = Principal + Rendimentos + VAAT (VAAT=0 no período)
        ...
        if tipo_70 == "Acumulado":
            ...
            # REGRA LINHA DE META: shapes com _OFFSET=0.22
        ...
        meta_eti_perc = 4.0
        # ETI = 0 no período; base = Principal + Rendimentos + VAAT + VAAR
        _base_eti = soma(
    ...
    # =========================================================================
        ...
        st.markdown("<h1 style='text-align:left;'>📖 Alpinópolis — Recursos Próprios (25%)</h1>",
                    unsafe_allow_html=True)
    ...
    # PNAE(1552), PNATE(1553), PTE(1576), QESE(1550,2550)
    # PTE: 'Tranferência Programas Estaduais' no R file.
    # Rendimentos bancários integrados à receita de cada programa.
    # =========================================================================
        ...
        # Rendimentos bancários por programa
        ...
        with _rv2: st.metric("Rendimentos Bancários", formar_real(total_rend_vinc))
        with _rv3: st.metric("Total Geral (Programas + Rendimentos)", formar_real(total_rec_vinc_geral))

    # =========================================================================
    # SETOR VISÃO MACRO
    # Capital Jan–Abr = R$ 83.357,21 | Custeio = R$ 6.654.813,23
    # =========================================================================
    elif st.session_state.setor == 'Visão Macro':
        st.markdown("<h1 style='text-align:left;'>📖 Alpinópolis — Visão Macro da Educação</h1>",
                    unsafe_allow_html=True)
        st.markdown("---")
    ...
    # =========================================================================
        ...
        st.markdown("<h1 style='text-align:left;'>📖 Alpinópolis — Folha de Pagamento</h1>",
                    unsafe_allow_html=True)
        st.markdown("---")
