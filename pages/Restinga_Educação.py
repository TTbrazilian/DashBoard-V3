...
# ── CARGA DE DADOS ────────────────────────────────────────────────────────────
# Individualidades Restinga vs Pratápolis:
# • Restinga.csv: cabeçalho duplo padrão [0,1] → colunas Mes_Liquidado/Empenhado/Pago.
# • meses_disponiveis = DINÂMICO (regra universal) → detecta Jan–Abr nesta base.
# • Restinga_R.csv: header=0 DIRETO (diferente de Pratápolis que tinha header duplo).
#   Coluna orçado = 'Orçado Receitas' (não 'Orçado'). Categorias com espaço no final
#   → usar .str.strip() em todos os filtros. 'Atualização Quadrimestral' existe mas
#   NÃO entra em COLS_R (evita double-parse).
# • PTE: NÃO tem linha no R file; apenas no DF (fonte 1576).
# • QESE fontes DF: ['1550','2550'] (2550 = Superávit QESE).
# • PNAE: R file + DF fonte 1552 | PNATE: R file + DF fonte 1553.
# • Restinga_DF.csv: estrutura limpa Tipo/Nomenclatura/Fonte, sem cabeçalho repetido.
# • _desconto_fundeb_nao_util = 0.0 | _desconto_superavit_ant = 0.0
# • Capital Liq Jan–Abr = R$ 53.280,65 | Custeio = R$ 5.510.667,78
# • FUNDEB 70% Liq Jan–Abr = R$ 3.292.332,31 | FUNDEB 30% Liq = R$ 249.794,19
# • RP 15001 Liq Jan–Abr = R$ 1.377.481,00
@st.cache_data
def load_all_data():
    path_f  = buscar_arquivo("zEducação/Restinga.csv")
    path_r  = buscar_arquivo("zEducação/Restinga_R.csv")
    path_df = buscar_arquivo("zEducação/Restinga_DF.csv")
    ...
    # ── RECEITAS — header=0 direto (diferente de Pratápolis) ─────────────────
    # Coluna orçado = 'Orçado Receitas'. Categorias com espaço no final → strip().
    ...
    # ── DESPESAS POR FONTE — estrutura limpa, sem cabeçalho repetido ──────────
...
# REGRA UNIVERSAL: meses_disponiveis SEMPRE dinâmico (nunca hardcoded).
# Um mês é considerado disponível se tiver qualquer valor não-nulo em FICHAS
# (Liquidado), no R file (receitas) ou no DF (despesas por fonte).
def _detectar_meses(df_f, df_r, df_df):
    meses = []
    for m in ORDEM_MESES:
        ...
        if df_f is not None and cl in df_f.columns and df_f[cl].abs().sum() > 0:
            tem = True
        if df_r is not None and m in df_r.columns and \
           pd.to_numeric(df_r[m], errors='coerce').abs().sum() > 0:
            tem = True
...
meses_disponiveis = _detectar_meses(df_f_raw, df_r, df_df_raw)
...
if df_f_raw is not None and df_r is not None:
    ...
    if st.session_state.setor == 'FUNDEB':
        st.markdown("<h1 style='text-align:left;'>📖 Restinga — FUNDEB</h1>",
                    unsafe_allow_html=True)
        ...
        # Categorias com espaço no final → strip() em todos os filtros
        ...
        # Restinga: coluna orçado = 'Orçado Receitas' | Portaria = Repasse
        ...
        # 'Atualização Quadrimestral' aplicado UMA VEZ sobre a string bruta
    ...
    # =========================================================================
        ...
        st.markdown("<h1 style='text-align:left;'>📖 Restinga — Recursos Próprios (25%)</h1>",
                    unsafe_allow_html=True)
        ...
        # Categorias com espaço no final → strip()
        ...
        # Restinga: coluna orçado = 'Orçado Receitas'
    ...
    # Restinga: PNAE(1552), PNATE(1553), PTE(1576), QESE(1550,2550)
    # PTE não tem linha no R file — só no DF.
    # QESE fontes DF: ['1550','2550'] (Superávit QESE = 2550)
    # =========================================================================
    elif st.session_state.setor == 'Recursos Vinculados':
        st.markdown("<h1 style='text-align:left;'>📖 Restinga — Recursos Vinculados</h1>",
                    unsafe_allow_html=True)
        ...
        # Vinculados no R file: categoria com espaço → strip()
        ...
        for prog in programas:
            # PTE pode não ter linha no R file
            df_prog_r = df_r_vinc[
    ...
    # =========================================================================
    # SETOR VISÃO MACRO
    # Capital Jan–Mar = R$ 53.280,65 | Custeio = R$ 3.909.218,53
    # =========================================================================
    elif st.session_state.setor == 'Visão Macro':
        st.markdown("<h1 style='text-align:left;'>📖 Restinga — Visão Macro da Educação</h1>",
                    unsafe_allow_html=True)
        st.markdown("---")
    ...
    # =========================================================================
        ...
        st.markdown("<h1 style='text-align:left;'>📖 Restinga — Folha de Pagamento</h1>",
                    unsafe_allow_html=True)
        st.markdown("---")
        ...
        # REGRA UNIVERSAL — Folha Gráfico 2: UMA barra por elemento (Total),
        # detalhamento por origem APENAS no hover (nunca empilhar por origem).
