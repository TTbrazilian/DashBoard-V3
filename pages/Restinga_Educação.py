...
if df_f_raw is not None and df_r is not None:
    ...
    # =========================================================================
        ...
        st.markdown("<h1 style='text-align: left;'>📖 Restinga - Recursos Próprios (25%)</h1>", unsafe_allow_html=True)

        # 1. Base de Cálculo: Impostos e Cota-Parte
        # Nota: 'Cota-Parte ' e 'Dedução ' possuem espaço ao final no arquivo de Restinga
        df_r_base = df_r[df_r['Categoria'].str.strip().isin(['Impostos', 'Cota-Parte'])].copy()

        # 2. Dedução FUNDEB — categoria 'Dedução ' (com espaço)
        ...
        # 3. Despesas 15001 — uma linha por tipo no arquivo de Restinga
        ...
        with m3: metric_contabil("Índice de Aplicação (Mín. 25%)", perc_25, 25.0)
        ...
        if view_meta == "Acumulado":
            ...
            fig_meta.add_hline(y=total_rec_base * 0.25, line_dash="dash", line_color="#f39c12",
                               annotation_text="Meta Constitucional (25%)", annotation_position="top left")
        else:
            dados_meta_m = []
            for m in meses_disponiveis:
                r_m   = df_r_base[m].sum()
            ...
            fig_meta.add_trace(go.Scatter(x=df_linha_meta['Mês'], y=df_linha_meta['Meta Mensal (25%)'],
    ...
    # =========================================================================
        ...
        st.markdown("<h1 style='text-align: left;'>📖 Restinga - Recursos Vinculados</h1>", unsafe_allow_html=True)

        # Fontes vinculadas presentes em Restinga (sem pares 2xxx com valores)
        mapa_desp = {
            'PNAE':  ['1552'],
            'PNATE': ['1553'],
            'PTE':   ['1576'],
